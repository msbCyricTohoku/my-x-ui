...
if [[ -f /etc/redhat-release ]]; then
    ...
    LOGE "System version not detected, please contact script author!\n" && exit 1
fi
...
confirm_restart() {
    confirm "Restart panel? Restarting panel will also restart xray" "y"
    if [[ $? == 0 ]]; then
        restart
    else
        show_menu
...
update() {
    confirm "This will reinstall the latest version, data will not be lost. Continue?" "n"
    if [[ $? != 0 ]]; then
        LOGE "Cancelled"
        if [[ $# == 0 ]]; then
            before_show_menu
...
uninstall() {
    confirm "Uninstall panel? xray will also be removed" "n"
    if [[ $? != 0 ]]; then
        if [[ $# == 0 ]]; then
            show_menu
        fi
    ...
    echo ""
    echo -e "Uninstall successful. To remove this script, run ${green}rm /usr/bin/x-ui -f${plain} after exiting"
    echo ""
...
reset_config() {
    confirm "Reset all panel settings? Account data will not be lost, username and password will not change" "n"
    if [[ $? != 0 ]]; then
        if [[ $# == 0 ]]; then
            show_menu
        fi
    ...
    echo -e "All panel settings reset to default. Please restart panel and use default port ${green}54321${plain}"
    confirm_restart
}
...
set_port() {
    ...
    if [[ -z "${port}" ]]; then
        ...
        echo -e "Port set. Please restart the panel and use the new port ${green}${port}${plain}"
        confirm_restart
    fi
}
...
start() {
    ...
    if [[ $? == 0 ]]; then
        echo ""
        LOGI "Panel is running, no need to start again. Choose restart if needed"
    else
        ...
        if [[ $? == 0 ]]; then
            ...
            LOGE "Panel failed to start, possibly because start time exceeded two seconds; check logs later"
        fi
    fi
...
stop() {
    ...
    if [[ $? == 1 ]]; then
        ...
        if [[ $? == 1 ]]; then
            ...
            LOGE "Panel failed to stop, possibly because stop time exceeded two seconds; check logs later"
        fi
    fi
...
restart() {
    ...
    if [[ $? == 0 ]]; then
        ...
        LOGE "Panel restart failed, possibly because start time exceeded two seconds; check logs later"
    fi
    if [[ $# == 0 ]]; then
        before_show_menu
    fi
...
enable() {
    ...
    if [[ $? == 0 ]]; then
        LOGI "x-ui enabled on startup"
    else
        LOGE "x-ui enable on startup failed"
    fi
...
disable() {
    systemctl disable x-ui
    if [[ $? == 0 ]]; then
        LOGI "x-ui disabled on startup"
    else
        LOGE "x-ui disable on startup failed"
    fi
...
update_shell() {
    ...
    if [[ $? != 0 ]]; then
        echo ""
        LOGE "Failed to download script, check if machine can connect to Github"
        before_show_menu
    else
        chmod +x /usr/bin/x-ui
        LOGI "Script updated successfully, please rerun" && exit 0
    fi
}
...
check_uninstall() {
    ...
    if [[ $? != 2 ]]; then
        echo ""
        LOGE "Panel already installed, do not reinstall"
        if [[ $# == 0 ]]; then
            before_show_menu
        fi
        return 1
...
ssl_cert_issue() {
    echo -E ""
    LOGD "******Usage******"
    LOGI "This script uses the Acme script to apply for certificates; ensure:"
    LOGI "1. Know Cloudflare registration email"
    LOGI "2. Know Cloudflare Global API Key"
    LOGI "3. Domain has been resolved to this server through Cloudflare"
    LOGI "4. Default certificate installation path is /root/cert"
    confirm "I confirm the above [y/n]" "y"
    if [ $? -eq 0 ]; then
        cd ~
        ...
        if [ $? -ne 0 ]; then
            LOGE "Failed to change default CA to Let's Encrypt, exiting"
            exit 1
        fi
...
show_usage() {
    ...
    echo "x-ui enable       - enable x-ui on startup"
    echo "x-ui disable      - disable x-ui on startup"
...
--------------------------------
  ...
  ${green}16.${plain} Apply for SSL certificate (acme)
 "
    show_status
