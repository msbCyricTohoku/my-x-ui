...
func (s *Server) initI18n(engine *gin.Engine) error {
	...
	findI18nParamNames := func(key string) []string {
		...
		for i := 0; i < keyLen-1; i++ {
			if key[i:i+2] == "{{" { // check for starting "{{"
				j := i + 2
				isFind := false
				for ; j < keyLen-1; j++ {
					if key[j:j+2] == "}}" { // ending "}}"
...
func (s *Server) startTask() {
	...
	go func() {
		...
		// count traffic every 10 seconds; initial start delayed 5 seconds to offset xray restart
		s.cron.AddJob("@every 10s", job.NewXrayTrafficJob())
	}()

	// check inbound traffic usage and expiry every 30 seconds
	s.cron.AddJob("@every 30s", job.NewCheckInboundJob())
	// remind traffic usage once per day at 08:30 Shanghai time
